...
cal_signal <- function(df) {
  ...
  data <- data %>% mutate(
    s_bull_stick = f_bull_stick(open, close),
  ...
  out <- tryCatch(
    {
      ...
      df.filtered.high.threshold <- sapply(df.filtered.high, function(x) {
        (y <- ifelse(x >= threshold, x, NA))
      })

      df.filtered.high.threshold.median <- apply(df.filtered.high.threshold, 2, median, na.rm = TRUE)
      df.filtered.high.threshold.count <- apply(df.filtered.high.threshold, 2, function(c) {
        sum(c != 0, na.rm = TRUE) / signal_count
      })
      df.filtered.high.threshold.return <- rbind(df.filtered.high.threshold.median, df.filtered.high.threshold.count)
      df.filtered.high.threshold.return[] <- vapply(df.filtered.high.threshold.return, function(x) {
        ifelse(is.na(x), 0, x)
      }, numeric(1))

      # Calculate negative return, median & count
      df.filtered.low <- df.filtered %>% select(matches("low.return"))
      df.filtered.low.threshold <- sapply(df.filtered.low, function(x) {
        (y <- ifelse(x <= -1 * threshold, x, NA))
      })

      df.filtered.low.threshold.median <- apply(df.filtered.low.threshold, 2, median, na.rm = TRUE)
      df.filtered.low.threshold.count <- apply(df.filtered.low.threshold, 2, function(c) {
        sum(c != 0, na.rm = TRUE) / signal_count
      })
      df.filtered.low.threshold.return <- rbind(df.filtered.low.threshold.median, df.filtered.low.threshold.count)
      df.filtered.low.threshold.return[] <- vapply(df.filtered.low.threshold.return, function(x) {
        ifelse(is.na(x), 0, x)
      }, numeric(1))
...
load_hit_signal <- function(ref.date, format = "long", option.only = TRUE, local = FALSE) {
  ...
  if (nrow(df.raw) == 0) {
    ...
    # Try yesterday if the result set is empty
    ref.date.ytd <- lubridate::ymd(ref.date) - 1
    query <- sprintf(" SELECT history.*
                     FROM SIGNAL_HISTORY history
                     WHERE DATE = '%s' ORDER BY CODE ASC", ref.date.ytd)

    df.raw <- sql_query(query, local)

    if (nrow(df.raw) == 0) {
      stop_quietly(sprintf("No data for date %s and %s", ref.date, ref.date.ytd))
    }
  }
...
get_signal_performance <- function(code, local = FALSE, verbose = FALSE) {
  ...
  # Derive day0 - day5
  res[, `:=`(
    day.0 = close, # Day 0 always closing price
